...
router.post("/", checkUser, async (req, res) => {
  ...
  try {
    ...
    const cartItem = req.user.cart.find(
      (item) => item.item.toString() === dishId
    );
    if (cartItem) {
    ...
    res
      .status(200)
      .json({ message: "Dish added to cart", cart: req.user.cart });
  } catch (error) {
...
// Delete cart item
router.delete("/delete/:dishId", checkUser, async (req, res) => {
  try {
    ...
    req.user = await User.findById(req.user.id).populate("cart.item");

    const cartIndex = req.user.cart.findIndex(item => item.item._id.toString() === dishId)
    if (cartIndex === -1)
...
router.delete("/deletecart", checkUser, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete cart", details: err.message });
  }
});
